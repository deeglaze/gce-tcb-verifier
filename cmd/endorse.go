...
func (f *endorseCommand) AddFlags(cmd *cobra.Command) {
	...
	cmd.PersistentFlags().BoolVar(&ec.Tdx.IncludeEarlyAccept, "tdx_include_early_accept", false, "DEPRECATED: If true, adds MRTD values for all supported machine shapes with all memory configured for acceptance in the firmware.")
	cmd.PersistentFlags().StringSliceVar(&ec.Tdx.MachineShapes, "tdx_machine_shapes", nil, "DEPRECATED: GCE machine shapes whose measurement-relevant configuration should be enumerated for endorsement.")
